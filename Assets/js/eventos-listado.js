(function(){
  ...
  const base = form.dataset.base || (window.BASE_URL + 'Eventos/index/');
  const list = document.getElementById('evt-list');
  const pag  = document.getElementById('evt-pag');
  const cnt  = document.getElementById('evt-counter');

  let timer = null;

  function qs(formEl){ return new URLSearchParams(new FormData(formEl)).toString(); }
  function buildUrl(page=1){ return `${base}${page}?${qs(form)}&ajax=1`; }
  function loading(on){ list.style.opacity = on ? '0.5' : ''; }
  ...
  async function update(page=1, push=true){
    ...
    try {
      ...
      if (cnt && typeof json.total === 'number') {
        ...
        cnt.textContent = total>0 ? `Mostrando ${ini}–${fin} de ${total} eventos` : 'No hay eventos con ese filtro';
      }
  ...
  form.querySelectorAll('select, input[type="date"]').forEach(el=>{
    el.addEventListener('change', ()=>update(1, true));
  });
  ...
  document.addEventListener('click', (e)=>{
    const a = e.target.closest('#evt-pag a.page-link');
    if (!a) return;
